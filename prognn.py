...
class ProGNN:
    ...
    def fit(self, g_data):
        ...
        estimator = EstimateAdj(adj, symmetric=args.symmetric, device=self.device).to(self.device)  # 将初始图结构设置为受扰图的邻接矩阵
    ...
    def train_gnn(self, adj, features, labels, epoch, criterion):
        ...
        if args.is_auxilary:
            ...
            f1_test = f1_score(torch.argmax(output[test_idx], dim=1).cpu().numpy(),
                               labels[test_idx].detach().cpu().numpy(), average='macro')
    ...
    def test(self, features, idx_test, labels):
        ...
        with torch.no_grad():
            for i in range(5):
                ...
                macrof1_test += f1_score(torch.argmax(output[idx_test], dim=1).cpu().numpy(),
                                         labels[idx_test].detach().cpu().numpy(), average='macro')
