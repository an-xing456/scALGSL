...
parser.add_argument('--data_dir', type=str,
                    default='../experiments/bcp1_6000-bcp2_6000-exp0050/data',
                    help='data directory')
parser.add_argument('--output_dir', type=str, default='../result/acc.csv', help='output directory')
'''
    Active Learning
'''
...
parser.add_argument('--gt_interval', type=int,
                    default=5,
                    help='interval for GSL')

parser.add_argument('--alpha', type=float,
                    default=0,  # LDS settings
...
# load data
g_data, auxilary_g_data, adata, data_info = load_data(args=args)
max_nodes_num = data_info['class_num'] * args.max_per_class
data_info['max_nodes_num'] = max_nodes_num

# For debug information
print("data path is {:}, \n ref_data num: {:}, \nquery_data num :{:}, \n auxilary data num:{:}".format(args.data_dir,
                                                                                                       adata.uns[
                                                                                                           'n_ref'],
                                                                                                       adata.uns[
                                                                                                           'n_query'],
                                                                                                       auxilary_g_data.num_nodes() ))
...
output_file_path = os.path.join('result', acc_file)
columns = ["acc"]
if not os.path.exists(output_file_path):
    acc_data = pd.DataFrame(columns=columns)
    acc_data.to_csv(output_file_path, index=False)
else:
    acc_data = pd.read_csv(output_file_path, index_col=0)
...
acc_data.to_csv(output_file_path)
...
cell_states = ['Angiogenesis', 'Apoptosis', 'CellCycle', 'Differentiation', 'DNAdamage', 'DNArepair', 'EMT',
