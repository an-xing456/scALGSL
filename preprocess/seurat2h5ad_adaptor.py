...
new_ref_data = create_adata(norm_ref_data.transpose(), ref_name['cell_name'].tolist(), genes,
                            ref_label['cell_type'].tolist(), False)
new_query_data = create_adata(norm_query_data.transpose(), query_name['cell_name'].tolist(), genes,
                              query_label['cell_type'].tolist(), False)
if norm_auxilary_data is not None:
    new_auxilary_data = create_adata(norm_auxilary_data.transpose(), auxilary_data.obs_names.tolist(), genes,
                                     auxilary_data.obsm['label'], True)
...
# Specify the root directory to be recursively deleted and the matching regular expression
